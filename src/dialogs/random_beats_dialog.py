...
class RandomBeatsDialog(Adw.Dialog):
    ...
    def _on_generate_clicked(self, _button):
        ...
        per_part_density = {}
        for part, controls in self._part_controls.items():
            _override_switch, scale, _label = controls
            # Always include per-part density if it differs from global
    ...
    def _wire_per_part_controls_from_ui(self):
        ...
        mapping = {
            ...
            "snare": (self.snare_scale, self.snare_value_label),
            "snare-2": (self.snare_2_scale, self.snare_2_value_label),
            "hihat": (self.hihat_scale, self.hihat_value_label),
            "hihat-2": (self.hihat_2_scale, self.hihat_2_value_label),
            "clap": (self.clap_scale, self.clap_value_label),
